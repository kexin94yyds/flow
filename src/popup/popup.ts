...
function renderCards(cards: Card[]): void {
  const list = document.getElementById('card-list') as HTMLDivElement;
  ...
  list.querySelectorAll('.card').forEach(el => {
    el.addEventListener('click', (e) => {
      ...
      if (card) chrome.tabs.create({ url: card.url });
    });
  });
...
function setupListeners(): void {
  const saveBtn = document.getElementById('save-current') as HTMLButtonElement;
